...
import (
	...
	"github.com/nats-io/nats.go/internal/jetstream"
)
...
func (js *js) AddConsumer(stream string, cfg *ConsumerConfig) (*ConsumerInfo, error) {
	...
	if cfg != nil && cfg.Durable != _EMPTY_ {
		...
		ccSubj = fmt.Sprintf(jetstream.APIDurableCreateT, stream, cfg.Durable)
	} else {
		ccSubj = fmt.Sprintf(jetstream.APIConsumerCreateT, stream)
	}
...
func (js *js) DeleteConsumer(stream, durable string) error {
	...
	dcSubj := js.apiSubj(fmt.Sprintf(jetstream.APIConsumerDeleteT, stream, durable))
...
func (c *ConsumerLister) Next() bool {
	...
	req, err := json.Marshal(consumersRequest{
		APIPagedRequest: jetstream.APIPagedRequest{Offset: c.offset},
	})
	if err != nil {
		c.err = err
		return false
	}
	clSubj := c.js.apiSubj(fmt.Sprintf(jetstream.APIConsumerListT, c.stream))
...
func (js *js) AddStream(cfg *StreamConfig) (*StreamInfo, error) {
	...
	csSubj := js.apiSubj(fmt.Sprintf(jetstream.APIStreamCreateT, cfg.Name))
...
func (js *js) StreamInfo(stream string) (*StreamInfo, error) {
	csSubj := js.apiSubj(fmt.Sprintf(jetstream.APIStreamInfoT, stream))
...
func (js *js) UpdateStream(cfg *StreamConfig) (*StreamInfo, error) {
	...
	usSubj := js.apiSubj(fmt.Sprintf(jetstream.APIStreamUpdateT, cfg.Name))
...
func (js *js) DeleteStream(name string) error {
	...
	dsSubj := js.apiSubj(fmt.Sprintf(jetstream.APIStreamDeleteT, name))
...
func (js *js) DeleteMsg(name string, seq uint64) error {
	...
	dsSubj := js.apiSubj(fmt.Sprintf(jetstream.APIMsgDeleteT, name))
...
func (js *js) PurgeStream(name string) error {
	psSubj := js.apiSubj(fmt.Sprintf(jetstream.APIStreamPurgeT, name))
...
func (s *StreamLister) Next() bool {
	...
	req, err := json.Marshal(streamNamesRequest{
		APIPagedRequest: jetstream.APIPagedRequest{Offset: s.offset},
	})
	if err != nil {
		s.err = err
		return false
	...
	slSubj := s.js.apiSubj(jetstream.APIStreamList)
