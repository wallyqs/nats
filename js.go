...
// Internal struct for JetStream context.
type js struct {
	nc *Conn
...
func (nc *Conn) JetStream(opts ...JSOpt) (JetStreamContext, error) {
	...
	js := &js{nc: nc, pre: jetstream.DefaultAPIPrefix, wait: defaultRequestWait}
	...
	resp, err := nc.Request(js.apiSubj(jetstream.APIAccountInfo), nil, js.wait)
	if err != nil {
...
func (js *js) lookupStreamBySubject(subj string) (string, error) {
	...
	resp, err := js.nc.Request(js.apiSubj(jetstream.APIStreams), j, js.wait)
	if err != nil {
		if err == ErrNoResponders {
			err = ErrJetStreamNotEnabled
		}
...
func (sub *Subscription) Poll() error {
	...
	reqNext := js.apiSubj(fmt.Sprintf(jetstream.APIRequestNextT, stream, consumer))
	return nc.PublishRequest(reqNext, reply, req)
}

func (js *js) getConsumerInfo(stream, consumer string) (*ConsumerInfo, error) {
	ccInfoSubj := fmt.Sprintf(jetstream.APIConsumerInfoT, stream, consumer)
