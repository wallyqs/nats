...
func TestJetStreamImportDirectOnly(t *testing.T) {
	...
	// BindOnly has the same effect as above since it would not attempt to create and lookup will work.
	sub, err = js.SubscribeSync("ORDERS", nats.BindOnly("ORDERS", "d4"))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	sub, err = js.PullSubscribe("ORDERS", "d1", nats.BindOnly("ORDERS"))
	if err != nil {
		t.Fatal(err)
	}
	expected := 10
...
func TestJetStreamBindConsumer(t *testing.T) {
	...
	_, err = js.SubscribeSync("foo", nats.BindOnly("foo", "push"))
	if err == nil || err != nil && !strings.Contains(err.Error(), "consumer not found") {
		t.Fatalf("Unexpected error: %v", err)
	}

	// Pull is the consumer
	_, err = js.PullSubscribe("foo", "pull", nats.BindOnly("foo"))
	if err == nil || err != nil && !strings.Contains(err.Error(), "consumer not found") {
		t.Fatalf("Unexpected error: %v", err)
	}
	...
	_, err = js.SubscribeSync("foo", nats.Durable("push2"), nats.BindOnly("foo", "push"))
	...
	_, err = js.PullSubscribe("foo", "push", nats.BindOnly("foo"))
	if err != nats.ErrPullSubscribeToPushConsumer {
		t.Fatalf("Unexpected error: %v", err)
	}
	_, err = js.SubscribeSync("foo", nats.BindOnly("foo", "pull"))
	...
	sub2, err := js.SubscribeSync("foo", nats.BindOnly("foo", cinfo.Name))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
