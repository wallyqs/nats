...
func TestJetStreamSubscribe(t *testing.T) {
	...
	if info.Config.AckWait != ackWait {
		t.Errorf("Expeced %v, got %v", ackWait, info.Config.AckWait)
	}
}

func TestJetStreamAckPendingGoesDown(t *testing.T) {
	s := RunBasicJetStreamServer()
	defer s.Shutdown()
...
func TestJetStreamAckPendingGoesDown2(t *testing.T) {
	s := RunBasicJetStreamServer()
	defer s.Shutdown()
	...
	// 3 messages delivered 5 times at least.
	var pending, recvd int
	expected := 15
	timeout := time.Now().Add(2 * time.Second)
	for time.Now().Before(timeout) {
	...
	for time.Now().Before(timeout) {
		...
		t.Logf("%+v", info)
		// fmt.Println(">>>", info.NumAckPending, info.NumRedelivered, info.NumWaiting, info.NumPending)
		if got, want := info.NumAckPending, 1; got > 0 && got != want {
			t.Fatalf("unexpected num ack pending: got=%d, want=%d", got, want)
		}
