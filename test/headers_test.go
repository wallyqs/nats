...
func TestMsgHeadersCasePreserving(t *testing.T) {
	...
	for _, key := range []string{"x-trace-id", "X-Trace-Id"} {
	...
	t.Run("multi value match", func(t *testing.T) {
		...
		// Case-insensitive first match would be the normalized canonical http v1 form.
		got = hdr.Get("fOo")
		if got != "B" {
			t.Errorf("Expected: %v, got: %v", expected, got)
		}

		// Values would match all much in the same way that http v1
		// normalizes multi-line headers with keys with different cases
		// into a single one.
		for _, key := range []string{"foo", "Foo", "FOO", "fOO", "foO"} {
			hdr = getHeader()
			result := hdr.Values(key)
			sort.Strings(result)
			...
			hdr.Del(key)
			if len(hdr) != 3 {
				t.Errorf("Expected to cleanup all matching keys, got: %v", len(hdr))
			}
			result = hdr.Values(key)
			if result != nil {
		...
		for _, key := range []string{"bar", "Bar", "BAR", "bAR", "baR"} {
			hdr = getHeader()
			result := hdr.Values(key)
			sort.Strings(result)

			expectedValues := []string{"D", "E", "F"}
			if !reflect.DeepEqual(result, expectedValues) {
				t.Errorf("Expected: %+v, got: %+v", expectedValues, result)
			}
			hdr.Del(key)
