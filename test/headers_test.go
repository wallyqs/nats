...
func TestMsgHeadersCasePreserving(t *testing.T) {
	...
	t.Run("via nats", func(t *testing.T) {
		subject := "headers.test"
		sub, err := nc.SubscribeSync(subject)
		if err != nil {
			t.Fatalf("Could not subscribe to %q: %v", subject, err)
	...
	t.Run("via http", func(t *testing.T) {
		// Need to create a copy of the message when re-publishing
		// a message that got delivered.
		// clone := func(m *nats.Msg) *nats.Msg {
		// 	return &nats.Msg{
		// 		Subject: m.Subject,
		// 		Reply:   m.Reply,
		...
		sub, err := nc.Subscribe("nats.svc.A", func(m *nats.Msg) {
			...
			msg.Header.Add("X-Result-A", "2.2.0")
		...
		sub, err = nc.Subscribe("nats.svc.B", func(m *nats.Msg) {
			...
			msg := &nats.Msg{
				...
				Header: http.Header{
					"Result-ID":  []string{"OK"},
					"X-Result-A": []string{"2.2.0"},
					"x-trace-id": []string{"S", "A"},
				},
			}
			...
			hdr = append(hdr, "B")
			msg.Header["x-trace-id"] = hdr
			msg.Header.Add("X-Result-B", "2.2.1")
		...
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			w.Header()["x-trace-id"] = []string{"S"}

			msg := nats.NewMsg("nats.svc.A")
			msg.Header = w.Header().Clone()
